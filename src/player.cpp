...
void Player::replaceThing(uint32_t index, Thing* thing)
{
	...
	Item* oldItem = inventory[index].get();
	if (!oldItem) {
		return /*RETURNVALUE_NOTPOSSIBLE*/;
	}
	...
	item->setParent(this);
	if (oldItem != item) { // Avoid releasing if same object
		itemPool.release(oldItem); // Return to pool
	}
	inventory[index] = std::unique_ptr<Item>(item); // Take ownership
}
...
void Player::removeThing(Thing* thing, uint32_t count)
{
	...
	if (item->isStackable()) {
		if (count == item->getItemCount()) {
			...
			itemPool.release(item); // Return to pool
...
uint64_t Player::getMoney() const
{
	...
	containers.reserve(CONST_SLOT_LAST + 1); // Initial guess
