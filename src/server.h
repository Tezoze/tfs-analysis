...
#include "signals.h"
#include <memory>
#include <boost/asio.hpp> // Explicitly include for io_context and related types
...
public:
	...
	// Public methods
	boost::asio::io_context& get_io_context() { return io_context; } // Added for ServiceManager::stop
	void onStopServer(); // Made public to allow access from ServiceManager
...
{
public:
	ServiceManager() : signals(std::make_unique<Signals>(io_context)), death_timer(io_context) {}
	~ServiceManager();
